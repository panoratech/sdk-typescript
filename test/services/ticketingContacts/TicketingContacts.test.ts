...
describe('test TicketingContacts', () => {
  ...
  describe('test getContacts', () => {
    test('test api call', () => {
      ...
      return sdk.ticketingContacts
        .getContacts('et', { remoteData: true })
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        ...
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.ticketingContacts.getContacts('nemo', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
  ...
  describe('test getContact', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/contacts/3591013252?remote_data=true')
        .reply(200, { data: {} });
      return sdk.ticketingContacts
        .getContact('3591013252', { remoteData: true })
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/contacts/9384070313?remote_data=true')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/contacts/1869090671?remote_data=true')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.ticketingContacts.getContact('1869090671', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
});
