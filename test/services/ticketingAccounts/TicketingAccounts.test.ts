...
describe('test TicketingAccounts', () => {
  ...
  describe('test getAccounts', () => {
    test('test api call', () => {
      ...
      return sdk.ticketingAccounts
        .getAccounts('maiores', { remoteData: true })
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        ...
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.ticketingAccounts.getAccounts('quis', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
  ...
  describe('test getAccount', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/8432652082?remote_data=true')
        .reply(200, { data: {} });
      return sdk.ticketingAccounts
        .getAccount('8432652082', { remoteData: true })
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/2291250009?remote_data=true')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/9025985495?remote_data=true')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.ticketingAccounts.getAccount('9025985495', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
});
