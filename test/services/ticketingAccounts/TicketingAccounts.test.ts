...
describe('test TicketingAccounts', () => {
  ...
  describe('test getAccounts', () => {
    test('test api call', () => {
      ...
      return sdk.ticketingAccounts
        .getAccounts('tempore', { remoteData: true })
  ...
  describe('test getAccount', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/5106927287?remote_data=true')
        .reply(200, { data: {} });
      return sdk.ticketingAccounts
        .getAccount('5106927287', { remoteData: true })
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/5409468902?remote_data=true')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/ticketing/accounts/7666823643?remote_data=true')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.ticketingAccounts.getAccount('7666823643', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
});
