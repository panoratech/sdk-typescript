  ...
  describe('test updateWebhookStatus', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .put('/webhook/6762730361')
        .reply(200, { data: {} });
      return sdk.webhook
        .updateWebhookStatus('6762730361')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .put('/webhook/5079039651')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .put('/webhook/9842587493')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.webhook.updateWebhookStatus('9842587493'),
      ).rejects.toThrow();
    });
  });
});
