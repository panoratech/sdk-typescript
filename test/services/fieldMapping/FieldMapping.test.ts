...
describe('test FieldMapping', () => {
  ...
  describe('test getCustomProviderProperties', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/field-mapping/properties?linkedUserId=velit&providerId=8038687720')
        .reply(200, { data: {} });
      return sdk.fieldMapping
        .getCustomProviderProperties('velit', '8038687720')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/field-mapping/properties?linkedUserId=fugiat&providerId=4754105731')
        .reply(200, { data: {} });
      return expect(
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/field-mapping/properties?linkedUserId=enim&providerId=4330888086')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.fieldMapping.getCustomProviderProperties('enim', '4330888086'),
      ).rejects.toThrow();
    });
  });
});
