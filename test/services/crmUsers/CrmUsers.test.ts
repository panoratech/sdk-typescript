...
describe('test CrmUsers', () => {
  ...
  describe('test getUsers', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        ...
        .reply(200, { data: {} });
      return sdk.crmUsers
        .getUsers('cupiditate', { remoteData: true })
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        ...
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.crmUsers.getUsers('alias', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
  ...
  describe('test getUser', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/crm/users/7492580874?remote_data=true')
        .reply(200, { data: {} });
      return sdk.crmUsers
        .getUser('7492580874', { remoteData: true })
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/crm/users/8155213779?remote_data=true')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/crm/users/3212849931?remote_data=true')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.crmUsers.getUser('3212849931', { remoteData: true }),
      ).rejects.toThrow();
    });
  });
});
