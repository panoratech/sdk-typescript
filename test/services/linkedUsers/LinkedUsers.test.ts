...
describe('test LinkedUsers', () => {
  ...
  describe('test getLinkedUser', () => {
    test('test api call', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/linked-users/single?originId=blanditiis')
        .reply(200, { data: {} });
      return sdk.linkedUsers
        .getLinkedUser('blanditiis')
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-dev.panora.dev')
        .get('/linked-users/single?originId=aut')
        .reply(404, { data: {} });
      return expect(async () => await sdk.linkedUsers.getLinkedUser('aut')).rejects.toThrow();
    });
  });
});
